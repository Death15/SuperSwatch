...
local function check_member(cb_extra, success, result)
   ...
   for k,v in pairs(result.members) do
      ...
      if member_id ~= our_id then
          ...
          return send_large_msg(receiver, 'You have been promoted as moderator for this supergroup.')
      end
    end
end
...
local function modlist(msg)
  ...
  if next(data[tostring(msg.to.id)]['moderators']) == nil then --fix way
    return 'No moderator in this supergroup.'
  end
  local message = 'List of moderators for Supergroup ' .. string.gsub(msg.to.print_name, '_', ' ') .. ':\n'
...
local mod_cmd = matches[1]
  ...
  if matches[1] == 'chat_add_user' and msg.action.from.id == our_id then
